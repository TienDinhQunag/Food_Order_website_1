    ...
    <body>
        ...
        <button onclick="searchOrderByStatus()">Tìm kiếm theo Status</button>
        <p id="search-result-message" style="display: none;"></p>
        ...
        <div class="cart">
            <table>
                ...
                <tbody>
                    ...
                    $sql = "SELECT DonHang.IDDonHang, DonHang.NgayDatHang, DonHang.TenTaiKhoan, DonHang.TrangThaiDonHang, DonHang.NgayGiaoHang
                            FROM DonHang";

                    if (!empty($_GET['statusFilter'])) {
                        $statusFilter = $_GET['statusFilter'];
                        $sql .= " WHERE DonHang.TrangThaiDonHang = '$statusFilter'";
                    }
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>";
                            echo "<form method='post' action='order_status_update.php'>";
                            echo "<input type='hidden' name='action' value='updateOrderStatus'>";
                            echo "<input type='hidden' name='orderId' value='" . $row['IDDonHang'] . "'>";
                            echo "<select name='newStatus'>";
                            echo "<option value='pending' " . ($row['TrangThaiDonHang'] == 'pending' ? 'selected' : '') . ">pending</option>";
                            echo "<option value='processing' " . ($row['TrangThaiDonHang'] == 'processing' ? 'selected' : '') . ">processing</option>";
                            echo "<option value='completed' " . ($row['TrangThaiDonHang'] == 'completed' ? 'selected' : '') . ">completed</option>";
                            echo "<option value='cancelled' " . ($row['TrangThaiDonHang'] == 'cancelled' ? 'selected' : '') . ">cancelled</option>";
                            echo "</select>";
                            echo "<button type='submit'>Cập nhật trạng thái</button>";
                            echo "</form>";
                            echo "</td>";
        ...
        <script>
            ...
            function searchOrderByStatus() {
                ...
                var searchUrl = "order_list.php?statusFilter=" + statusFilter;
